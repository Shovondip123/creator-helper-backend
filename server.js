...
function ensureUser(db,uid,profile={}) {
  if (!db.users[uid]) {
    db.users[uid] = {
      remainingContents: 3,
      credits: 3,
    ...
    Object.assign(db.users[uid], profile);
    if (!Number.isFinite(Number(db.users[uid].remainingContents))) {
      db.users[uid].remainingContents = Number.isFinite(Number(db.users[uid].credits))
        ? Math.max(0, Number(db.users[uid].credits))
        : 0;
    }
    if (!Number.isFinite(Number(db.users[uid].generations))) db.users[uid].generations = 0;
    if (!Number.isFinite(Number(db.users[uid].balanceBdt))) db.users[uid].balanceBdt = 0;
    if (!Number.isFinite(Number(db.users[uid].balanceUsd))) db.users[uid].balanceUsd = 0;
  }
  // Keep legacy credits synchronized for old frontend compatibility.
  db.users[uid].credits = Number(db.users[uid].remainingContents || 0);
  return db.users[uid];
}
...
app.get('/api/me',authMiddleware,requireVerified,async(req,res)=>{const db=await readDb();const u=ensureUser(db,req.user.uid,{name:req.user.name||req.user.displayName||'',email:req.user.email||'',phone:req.user.phone_number||''});await writeDb(db);res.json({uid:req.user.uid,name:u.name,email:u.email,phone:u.phone,remainingContents:Number(u.remainingContents||0),credits:Number(u.remainingContents||0),generations:Number(u.generations||0)});});
app.get('/api/user/credits',authMiddleware,requireVerified,async(req,res)=>{const db=await readDb();const u=ensureUser(db,req.user.uid);await writeDb(db);res.json({remainingContents:Number(u.remainingContents||0),credits:Number(u.remainingContents||0),generations:Number(u.generations||0)});});
app.post('/api/generate-content',authMiddleware,requireVerified,async(req,res)=>{
  const validationError=validateInput(req.body);
  if(validationError)return res.status(400).json({error:validationError});
  const db=await readDb();
  const u=ensureUser(db,req.user.uid);
  if(Number(u.remainingContents||0)<=0)return res.status(402).json({error:'No content remaining.',remainingContents:0,credits:0});
  try{
    const content=DEMO_MODE?demoContent(req.body):await callGemini(req.body);
    u.remainingContents=Math.max(0,Number(u.remainingContents||0)-1);
    u.credits=u.remainingContents;
    u.generations=Number(u.generations||0)+1;
    await writeDb(db);
    res.json({content,remainingContents:u.remainingContents,credits:u.remainingContents,generations:u.generations});
  }catch(e){
    console.error(e);
...
app.post('/api/payments',authMiddleware,requireVerified,async(req,res)=>{const name=String(req.body?.name||req.user.name||req.user.email||'').trim(),method=String(req.body?.method||'').trim(),reference=String(req.body?.reference||'').trim(),account=String(req.body?.account||'').trim(),note=String(req.body?.note||'').trim(),packageName=normalizePackageName(req.body?.packageName??req.body?.package),currency=String(req.body?.currency||'BDT').trim().toUpperCase(),amount=Number(req.body?.amount),allowedMethods=['bKash','Nagad','Skrill','ACH / Bank Transfer'];if(!name||!reference||!account)return res.status(400).json({error:'Name, sender/account details and payment reference are required.'});if(!allowedMethods.includes(method))return res.status(400).json({error:'Invalid payment method.'});if(!packageName)return res.status(400).json({error:'Please select a valid package.'});if(!['BDT','USD'].includes(currency))return res.status(400).json({error:'Currency must be BDT or USD.'});if(!Number.isFinite(amount)||amount<0.01||amount>10000000)return res.status(400).json({error:'Invalid deposit amount.'});const db=await readDb();ensureUser(db,req.user.uid,{name,email:req.user.email||'',phone:req.user.phone_number||''});db.payments??=[];const payment={id:`pay-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`,userId:req.user.uid,name,packageName,amount,currency,amountBdt:currency==='BDT'?amount:amount*USD_BDT_RATE,amountUsd:currency==='USD'?amount:amount/USD_BDT_RATE,method,reference,account,note,status:'pending',createdAt:new Date().toISOString()};db.payments.push(payment);await writeDb(db);res.status(201).json({ok:true,paymentId:payment.id,status:'pending',currency,amount});});
app.get('/api/admin/payments',adminGuard,async(_req,res)=>{const db=await readDb();res.json({payments:db.payments||[]});});
app.post('/api/admin/payments/:id/approve',adminGuard,async(req,res)=>{
  try{
    const result=await withPaymentLock(async()=>{
      const db=await readDb();
      const payment=(db.payments||[]).find(x=>x.id===req.params.id);
      if(!payment)return{status:404,body:{error:'Payment not found.'}};
      if(payment.status!=='pending')return{status:400,body:{error:'Only pending payments can be approved.'}};
      const packageName=normalizePackageName(payment.packageName||req.body?.packageName||req.body?.package);
      const packageContents=Number(PACKAGE_RULES[packageName]||0);
      if(!packageContents)return{status:400,body:{error:'Payment has no valid package.'}};
      const user=ensureUser(db,payment.userId);
      const currency=String(payment.currency||'BDT').toUpperCase();
      const amount=Number(payment.amount||0);
      if(currency==='USD')user.balanceUsd=Number(user.balanceUsd||0)+amount;
      else if(currency==='BDT')user.balanceBdt=Number(user.balanceBdt||0)+amount;
      else return{status:400,body:{error:'Unsupported payment currency.'}};
      user.remainingContents=Number(user.remainingContents||0)+packageContents;
      user.credits=user.remainingContents;
      payment.status='approved';
      payment.approvedAt=new Date().toISOString();
      payment.approvedPackageContents=packageContents;
      await writeDb(db);
      return{status:200,body:{ok:true,status:'approved',packageName,packageContents,remainingContents:Number(user.remainingContents||0),credits:Number(user.remainingContents||0),currency,amount,balanceBdt:Number(user.balanceBdt||0),balanceUsd:Number(user.balanceUsd||0)}};
    });
    res.status(result.status).json(result.body);
  }catch(e){
    console.error(e);
    res.status(500).json({error:'Approval could not be completed.'});
  }
});
